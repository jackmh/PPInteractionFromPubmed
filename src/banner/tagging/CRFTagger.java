...
public class CRFTagger implements Tagger {
	...
	private CRFTagger(CRF4 forwardCRF, CRF4 reverseCRF,
			String2TokenSequencePipe basePipe, int order,
			boolean useFeatureInduction, TagFormat format,
	 ...
	 */
	public static CRFTagger load(File f, Lemmatiser lemmatiser,
			dragon.nlp.tool.Tagger posTagger) throws IOException {
		try {
			...
			String2TokenSequencePipe basePipe = (String2TokenSequencePipe) ois
					.readObject();
			...
			ois.close();
			return new CRFTagger(forwardCRF, reverseCRF, basePipe, order,
					useFeatureInduction, format, textDirection);
	 ...
	 *         to disk
	 */
	public static CRFTagger train(List<Sentence> sentences, int order,
			boolean useFeatureInduction, TagFormat format,
			TextDirection textDirection, Lemmatiser lemmatiser,
			dragon.nlp.tool.Tagger posTagger, boolean useNumericalNormalization) {
		if (sentences.size() == 0)
			throw new RuntimeException(
					"Number of sentences must be greater than zero");
		String2TokenSequencePipe localBasePipe = new String2TokenSequencePipe(
				lemmatiser, posTagger, useNumericalNormalization);
		...
		if (textDirection == TextDirection.Intersection)
			throw new UnsupportedOperationException(
					"TextDirection.Intersection not yet supported");
		if (textDirection.doForward())
			forwardCRF = train(sentences, order, useFeatureInduction, format,
					pipe, false);
		CRF4 reverseCRF = null;
		if (textDirection.doReverse())
			reverseCRF = train(sentences, order, useFeatureInduction, format,
					pipe, true);
		return new CRFTagger(forwardCRF, reverseCRF, localBasePipe, order,
				useFeatureInduction, format, textDirection);
	}

	private static CRF4 train(List<Sentence> sentences, int order,
			boolean useFeatureInduction, TagFormat format, Pipe pipe,
			boolean reverse) {
		InstanceList instances = new InstanceList(pipe);
		for (Sentence sentence : sentences) {
			String text = sentence.getTrainingText(format, reverse);
			instances.add(new Instance(text, null, sentence.getTag(), null,
					pipe));
		}
		...
		if (useFeatureInduction)
			crf.trainWithFeatureInduction(instances, null, null, null, 99999,
					100, 10, 1000, 0.5, false, new double[] { .2, .5, .8 });
		else
			crf.train(instances, null, null, (MultiSegmentationEvaluator) null,
	...
	private void getPositionsAndTypes(Sentence sentence,
			TagPosition[] positions, MentionType[] types, boolean reverse) {
		Instance instance = new Instance(sentence.getTrainingText(format,
				reverse), null, sentence.getTag(), null,
				forwardCRF.getInputPipe());
		Sequence tags = forwardCRF.viterbiPath((Sequence) instance.getData())
				.output();
