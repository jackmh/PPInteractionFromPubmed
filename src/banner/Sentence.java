...
public class Sentence {
			...
			if (pieces[i].startsWith("<") && pieces[i].endsWith(">")) {
				...
				if (typeStr.startsWith("</")) {
					if (mentionStart == -1 || mentionType == null)
						throw new IllegalArgumentException("");
					Mention mention = new Mention(sentence, mentionType,
							mentionStart, index);
	...
	public void inferTokenization(String tokenizedText) {
		...
		for (int i = 0; i < split.length; i++) {
			while (Character.isWhitespace(text.charAt(start)))
				start++;
			if (!text.substring(start, start + split[i].length()).equals(
					split[i]))
				throw new IllegalArgumentException();
	...
	public void addOrMergeMention(Mention mention) {
		...
		for (Mention mention2 : mentions) {
			if (mention.overlaps(mention2)
					&& mention.getType().equals(mention2.getType()))
		...
		if (overlapping.size() == 0) {
			...
			for (Mention mention2 : overlapping) {
				...
				mentions.add(new Mention(this, mention.getType(), Math.min(
						mention.getStart(), mention2.getStart()), Math.max(
						mention.getEnd(), mention2.getEnd())));
			}
		}
	}
	...
	public void addMentions(TagPosition[] positions, MentionType[] types) {
		...
		for (int i = 0; i < positions.length; i++) {
			if (positions[i] == TagPosition.O) {
				if (startIndex != -1)
					addOrMergeMention(new Mention(this, types[i - 1],
							startIndex, i));
				startIndex = -1;
	...
	public String getSGML() {
		...
		for (int i = 0; i < taggedTokens.size(); i++) {
			...
			String text = token.getToken().getText();
			
			if (text.length() == 1 && SimpleTokenizer.isLeftPunctuation(text.charAt(0)))
			{
				text2 = new StringBuffer(text2.toString().trim());
			}
			
			text2.append(token.getToken().getText() + " ");
			
			if (text.length() == 1 && SimpleTokenizer.isRightPunctuation(text.charAt(0)))
			{
				text2 = new StringBuffer(text2.toString().trim());
			}
